...
packer.startup({
	function(use)
		...
		-- Installer
		use({ localplug("installer"), config = [[require'installer'.setup()]] })
		...
		use({ "kylechui/nvim-surround", config = [[require'modules.surround']], event = "BufRead" })
		use({ "windwp/nvim-autopairs", config = [[require'modules.autopairs']], event = "InsertEnter" })
		use({ "lewis6991/gitsigns.nvim", config = [[require'modules.gitsigns']], event = "BufRead" })
		use({
			"folke/trouble.nvim",
		...
		-- Syntax highlight
		use({ "chr4/nginx.vim", opt = true })

		-- Telescope
		use({ "nvim-telescope/telescope.nvim", config = [[require'modules.telescope']] })
		use({
		...
		-- FZF
		use({ "ibhagwan/fzf-lua", config = [[require'modules.fzf']] })

		-- Treesitter
		use({ "nvim-treesitter/nvim-treesitter", run = ":TSUpdate", config = [[require'modules.treesitter']] })
		...
		-- Snippets
		use({ "L3MON4D3/LuaSnip", config = [[require'modules.luasnip']], opt = true })
		...
		use({ "SmiteshP/nvim-navic", config = [[require'modules.navic']], module = "nvim-navic" })
		use({
